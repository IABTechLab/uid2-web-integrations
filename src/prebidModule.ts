...
function readCookie(): StoredValue | null {
    ...
    return storage.cookiesAreEnabled() ? JSON.parse(storage.getCookie(ADVERTISING_COOKIE)) as StoredValue : null;
}
...
function readFromLocalStorage(): StoredValue | null {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    return storage.localStorageIsEnabled() ? storage.getDataFromLocalStorage(ADVERTISING_COOKIE) as StoredValue : null;
}
